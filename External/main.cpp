...
void bhop() {
	int locFlags;
	//int locPlayer, entList;
	//bool jump;
	while (status[0]) {
		if (rpm<int>(clientMod + dwEntityList)) {
		//ReadProcessMemory(hProc, (PVOID)(clientMod + dwEntityList), &entList, sizeof(int), 0);
		//if (entList) {
			locFlags = rpm<int>(rpm<int>(clientMod + dwLocalPlayer) + m_fFlags);
			//ReadProcessMemory(hProc, (PVOID)(clientMod + dwLocalPlayer), &locPlayer, sizeof(int), 0);
			//ReadProcessMemory(hProc, (PVOID)(locPlayer + m_fFlags), &locFlags, sizeof(int), 0);
			if (locFlags == 257 && GetAsyncKeyState(VK_SPACE) || locFlags == 263 && GetAsyncKeyState(VK_SPACE))
				...
				//jump = false;
			//WriteProcessMemory(hProc, (PVOID)(clientMod + dwForceJump), &jump, sizeof(bool), 0);
...
int main() {
	...
	// cout << hProc << " " << procId << " " << clientMod << endl;
	...
	while (input != 27) {
		...
		for (int i = 1; i <= sizeof(names) / sizeof(names[0]); i++) {
			...
			for (int j = 0; j < max - names[i - 1].size() + 1; j++)
				cout << " ";
			cout << "<" << (status[i - 1] == true ? "On" : "Off") << ">" << endl;
		}
		input = _getch();
		switch (input) {
		case 72: if (choice > 1)
